...
{
    public partial class U19 : System.Web.UI.Page
    {
                ...
                using (MESLEntities txt = new MESLEntities())
                {
                    ...
                    foreach (TEAM _t in _tms)
                    {
                        ...
                        _teams.Add(new Local_Team
                        {
                            ...
                            PLAYER19 = (!string.IsNullOrEmpty(_t.PLAYER19.ToString())) ? (((from p in _players where p.ID == _t.PLAYER19 select p).First().GENDER == true) ? "(m)" : "(f)") + " " + (from p in _players where p.ID == _t.PLAYER19 select p).First().FNAME.Trim() + " " + (from p in _players where p.ID == _t.PLAYER19 select p).First().LNAME.Trim() + " [ " + (from p in _players where p.ID == _t.PLAYER19 select p).First().AGE.ToString("#.##") + " ]" : "",
                            PLAYER20 = (!string.IsNullOrEmpty(_t.PLAYER20.ToString())) ? (((from p in _players where p.ID == _t.PLAYER20 select p).First().GENDER == true) ? "(m)" : "(f)") + " " + (from p in _players where p.ID == _t.PLAYER20 select p).First().FNAME.Trim() + " " + (from p in _players where p.ID == _t.PLAYER20 select p).First().LNAME.Trim() + " [ " + (from p in _players where p.ID == _t.PLAYER20 select p).First().AGE.ToString("#.##") + " ]" : "",
        ...
        protected void ddlteam_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (MESLEntities ptx = new MESLEntities())
            {
                ...
                else if (string.IsNullOrEmpty(t.PLAYER19.ToString()))
                {
                    t.PLAYER19 = playerid;
                    p.TEAMID = teamid;
                    ptx.SaveChanges();
                ...
                else if (string.IsNullOrEmpty(t.PLAYER20.ToString()))
                {
                    t.PLAYER20 = playerid;
                    p.TEAMID = teamid;
                    ptx.SaveChanges();
                    ...
                    using (MESLEntities ttx = new MESLEntities())
                    {
                        ...
                        switch (teammembernumber)
                        {
                            ...
                            case 18:
                                ...
                                break;
                            case 19:
                                _p = (from p in ttx.PLAYERs where p.ID == _t.PLAYER19 select p).First();
                                _p.TEAMID = 1;
                                _t.PLAYER19 = null;
                                break;
                            case 20:
                                _p = (from p in ttx.PLAYERs where p.ID == _t.PLAYER20 select p).First();
                                _p.TEAMID = 1;
                                _t.PLAYER20 = null;
                                break;
                        }
